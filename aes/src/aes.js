...
const INV_S_BOX = [];
S_BOX.forEach((e, i) => INV_S_BOX[e] = i);
...
function mixColumns(state, coefficients = [2, 1, 1, 3]) {
  for (let col = 0; col < numColumns; col++) {
    ...
    state[0][col] = mul(i, a) ^ mul(l, b) ^ mul(k, c) ^ mul(j, d);
    state[1][col] = mul(j, a) ^ mul(i, b) ^ mul(l, c) ^ mul(k, d);
    state[2][col] = mul(k, a) ^ mul(j, b) ^ mul(i, c) ^ mul(l, d);
    state[3][col] = mul(l, a) ^ mul(k, b) ^ mul(j, c) ^ mul(i, d);
  }
}
