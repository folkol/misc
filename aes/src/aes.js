...
// Affine Transformation after Multiplicative inverse in GF(2**8), see §5.1.1
const S_BOX = [
...
function subBytes(state) {
  state.forEach((row, r) => row.forEach((b, c) => {
    state[r][c] = S_BOX[b];
  }));
}
...
function invShiftRow(row, n) {
  let prefix = row.splice(0, 4 - n);
  row.push(...prefix);
}

function invShiftRows(state) {
  state.forEach(invShiftRow);
}


// (Pre-computed lookup-tables from https://en.wikipedia.org/wiki/Rijndael_MixColumns)
const MUL_2 = [
...
function mixColumns(state) {
  for (let column = 0; column < numColumns; column++) {
    ...
    state[0][column] = mul(0x02, a) ^ mul(0x03, b) ^ mul(0x01, c) ^ mul(0x01, d);
    state[1][column] = mul(0x01, a) ^ mul(0x02, b) ^ mul(0x03, c) ^ mul(0x01, d);
    state[2][column] = mul(0x01, a) ^ mul(0x01, b) ^ mul(0x02, c) ^ mul(0x03, d);
    state[3][column] = mul(0x03, a) ^ mul(0x01, b) ^ mul(0x01, c) ^ mul(0x02, d);
  }
}
...
function invMixColumns(state) {
  for (let column = 0; column < numColumns; column++) {
    ...
    state[0][column] = mul(0x0e, a) ^ mul(0x0b, b) ^ mul(0x0d, c) ^ mul(0x09, d);
    state[1][column] = mul(0x09, a) ^ mul(0x0e, b) ^ mul(0x0b, c) ^ mul(0x0d, d);
    state[2][column] = mul(0x0d, a) ^ mul(0x09, b) ^ mul(0x0e, c) ^ mul(0x0b, d);
    state[3][column] = mul(0x0b, a) ^ mul(0x0d, b) ^ mul(0x09, c) ^ mul(0x0e, d);
  }
}
